...
return {
	...
	update = function(self, dt)
		...
		if input:pressed("gameLeft") or input:pressed("gameRight") or input:pressed("gameUp") or input:pressed("gameDown") then
			inputting = true
		elseif input:released("gameLeft") or input:released("gameRight") or input:released("gameUp") or input:released("gameDown") then
			inputting = false
		end
	...
	updateUI = function(self, dt)
				...
				if (not settings.downscroll and enemyNote[1].y - musicPos <= -400) or (settings.downscroll and enemyNote[1].y - musicPos >= 400) then
					...
					if enemyNote[1]:getAnimName() == "hold" or enemyNote[1]:getAnimName() == "end" then
						...
						if curAnim == "left" then
							self:safeAnimate(picoSpeaker, "left", false, 2)
						elseif curAnim == "up" then
							self:safeAnimate(picoSpeaker, "right", false, 2)
						end
						if useAltAnims then
		...
		elseif health <= 0 then -- Game over
			Gamestate.push(gameOverTankman)
